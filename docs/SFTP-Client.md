...
> **实现要点（macOS 实测）**：本地代理的 Range 响应必须是**带 `Content-Length` 的流式 206**；
> 缺 `Content-Length` 时播放器会把该输入判为不可 seek（拖进度条后不请求新位置的数据、暂停恢复直接 Ended）。
> 另外 `seekTo` 不可与播放进度共用一个状态（会每秒 seek 导致卡顿），播放器的待补发 seek 必须在进入可播状态后补发。
> 详见 `AGENTS.md` §13.4 第 10~14 条。
