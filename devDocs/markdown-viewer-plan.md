...
| 代码块 + 语法高亮 | ● | ● | ● | 已具备：commonMain 轻量 tokenizer `CodeHighlighter`（关键字/字符串/注释/数字/函数/类型/属性，多语言别名归一；**禁用正则**）；用例 T15 |
| 目录 TOC | ● | ● | ● | 二级弹窗（近似跳转） |
...
| 代码语法高亮 | ● | ● | ● | 已落地：commonMain 轻量 tokenizer（六端共用）；宿主 highlight.js 仍可作为可选增强 |
