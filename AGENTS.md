...
## 2.5 协作与决策规则（AI 自主决策，用户已授权）

**用户已明确授权：本仓库的技术与实现决策全部由 AI 自主决定，不必逐项征求确认。**
用户不再逐行审阅代码，所以 AI 必须把「自己判断 → 自己验证 → 自己收尾 → 如实汇报」整条链路做完整，
不能把本该自己定的问题抛回给用户。

1. **自主决策范围**：技术选型、代码实现与重构、文件增删、依赖引入与 vendor、构建/测试修复、
   提交粒度与提交信息、`AGENTS.md` / `devDocs` 文档更新、是否提交与推送、任务拆分与优先级。
   **其中编辑 `AGENTS.md` 本身无需询问、直接自动允许**（已在仓库根 `kilo.json` 的
   `permission.edit` 配置 `AGENTS.md` / `**/AGENTS.md` 为 `allow`；其它文件权限不变）。
2. **不要再问的典型问题**：命名、目录结构、用哪个库、异常怎么处理、跨端差异如何取舍、
   要不要拆提交、要不要补用例 —— 一律 AI 拍板，并把理由写进提交信息或文档。
3. **必须自己验证**：改完要跑受影响的端（遵循 §3.1 第 9 条「按影响面选测，不做全量回归」），
   禁止「改完不验证就交付」。确实无法验证的（如本机无设备/模拟器的端）要**如实标注「未运行时验证」**，
   不得把编译通过说成功能可用。
4. **汇报格式**：每轮收尾用简明中文说明「改了什么功能 / 修了什么 / 验证到什么程度 / 还剩什么」，
   结论先行，不粘贴大段代码与日志。
5. **仍然保留的红线（不因授权而放开）**：
   - 不得提交密钥、凭据、个人隐私；测试机凭据按 §13.6 处理，不写进会公开的文档。
   - 不得执行不可逆的破坏性操作（`push --force`、`reset --hard` 丢掉他人改动、`rm -rf` 工作区、
     删他人分支），除非用户明确要求。
   - 不得擅自改动权限、签名、发布/分发配置（**用户已明确授权**的 `AGENTS.md` 编辑放行除外，见第 1 条）。
   - 不得伪造验证结果（没跑就说跑了、编译通过说成功能可用）。
6. **出错自己修**：自己引入的失败（构建/用例挂）自行排查到通过；确属环境或外部依赖阻塞的，
   给出证据与可选下一步，而不是原样抛回给用户。
7. **仅 `zhaojian` 分支有固定交付动作（打 dmg + 覆盖安装）**：在 `zhaojian` 上每次功能开发/修复收尾**必须**
   「打 dmg → `~/Downloads` 留副本 → 覆盖安装到 `/Applications/Kuikly SFTP.app`」，命令与细节见 §3.1 规则 11。
   这是本机验证与回滚的唯一可信基线。
   **构建默认用 debug 产物**（`npm run build:web` + `npm run dist`，快）；`npm run dist:release` 太慢，
   **仅 `zhaojian` 分支**且确有需要（正式发版）时才跑（见 §3.1 规则 16）。
   **其它分支（feature/agent/`.kilo/worktrees/*`）只做验证**：跑受影响用例 + 编译通过即可，**不构建 dmg、不覆盖安装**
   （避免全局动作互相踩踏，本机应用只有一份）；成果按规则 7 同步到 `zhaojian` 后，由 `zhaojian` 侧统一打包安装。
...
7. **主工作分支 = `zhaojian`，开发前先同步、成果要及时同步回去**：主 clone `/Users/zhaojian/bin/macmini/KuiklyUI` 检出在 `zhaojian`；
   ...
   - **同步遇冲突必须先解决冲突，再验证，最后才同步**：向 `zhaojian` 同步前若 rebase/merge 出现冲突，
     ...
     未解决冲突或未验证通过时，**绝不推送**；无法解决/不确定时报告用户。
   ...
   - 冲突 / 远端分叉 / 不确定时**绝不强行合并**，报告用户（`sync-main.mjs` 退出码：`0` 已同步、`2` 需等待、
     `3` 超时跳过、`4` 分叉需人工、`6` 冲突需人工、`1` 本 worktree 有未提交改动）。
...
7. **单步不得过长，及时止损**：任何一步（构建/安装/单条用例/网络请求）都必须有明确超时；耗时明显偏离正常
...
11. **仅 `zhaojian` 分支执行「打 dmg + 存 Downloads + 覆盖安装到本机」**（固定交付动作，不可省；**其它分支一律不做**）：
   ...
   （`zhaojian` 侧）仅当用户明确说「不用打包/不用安装」时才可跳过；跳过时必须在收尾汇报里写明原因。
...
15. **每个 worktree 完成一项任务时，必须把「未提交的代码」列出来告诉用户**：
   - 任务收尾（交付/汇报前）执行 `git status --short`，必要时 `git diff --stat`，把**未提交（含未跟踪）的文件清单 + 改动摘要**
     明确写进结果里，让用户清楚哪些还没提交。
...
## 14.1 SFTP 桌面端（Electron）固定交付流程（**仅 `zhaojian` 分支执行**）

**触发条件**：在 **`zhaojian` 分支**上，任何一轮「测试用例全部通过 + 问题修复完成」之后。
**其它分支（feature/agent/`.kilo/worktrees/*`）不执行本节**：只跑受影响用例 + 编译通过，把成果按规则 7 同步到
`zhaojian` 后，由 `zhaojian` 侧统一打包安装（见 §3.1 规则 11）。
...
# ① 打包（默认 debug 产物，快；release 太慢，仅 zhaojian 且确有需要时用）
npm run build:web && npm run dist         # debug web → sync → electron-builder → dist/Kuikly SFTP-0.1.0.dmg
# 仅正式发版才用（慢）：npm run dist:release   # = build:web:release + sync(release) + electron-builder
